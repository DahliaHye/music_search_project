"""
librosa 기반 콘텐츠 특징: 믹스 전체 + 보컬 스템 전용 벡터를 결합합니다.
"""
...
def extract_all(
    ...
    *,
    ...
    """
    믹스 특징 + 보컬 분리 스템 특징 + 결합 벡터 + 도출 파라미터.
    """
    import librosa
    ...
    y_v, sep_info = separate_vocals(y, sr, audio_path, max_seconds=max_seconds)
...
def extract_features(
    ...
    *,
    ...
    """
    하위 호환: (믹스+보컬 flat 병합, 결합 벡터) 반환.
    """
    out = extract_all(audio_path, sr=sr, max_seconds=max_seconds)
    flat = {**out["flat_mix"], **out["flat_vocal"]}
    return flat, out["vec_combined"]
